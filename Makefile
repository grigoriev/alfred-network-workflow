...
REPO         := grigoriev/alfred-network-workflow
...
.PHONY: all build updater verify-updater test coverage lint clean \
	print-artifact print-release-files print-provenance print-version set-version verify-js
...
test: updater verify-js
	bats tests

# Confirm the Wi-Fi scanner produces valid JSON (macOS, runs with the tests)
verify-js:
...
# for SonarCloud. kcov needs Linux, so it runs in its container.
# kcov mangles a few jq-heavy tests that pass on macOS and under bats directly,
# so `|| true` keeps coverage flowing. The macOS test job still validates those
# lines; coverage lands around 90%. The Wi-Fi scanner tests need macOS.
coverage: updater
	docker run --rm -v "$(CURDIR):$(CURDIR)" -w "$(CURDIR)" $(KCOV_IMAGE) bash -c \
		"apt-get update -qq && apt-get install -y -qq bats jq >/dev/null && kcov --include-pattern=src/ coverage bats \$$(ls tests/*.bats | grep -v wifi_scan_tests) || true"
